...
```text
从 Office 元数据编辑器 (Office Metadata Editor)
升级为 MetaDocu 隐私扫描器 (MetaDocu Privacy Scanner)
```
...
```text
Office 元数据编辑器 (Office Metadata Editor)
...
MetaDocu 免费本地隐私元数据扫描器 (MetaDocu Free Local Metadata Privacy Scanner)
```
...
export function useWasmParser() {
  ...
  useEffect(() => {
    async function initWasm() {
      try {
        const wasm = await import("@/lib/wasm/metadocu_parser_wasm");
...
```typescript
import initWasm, { parse_docx_wasm, clean_docx_wasm, deallocate_wasm_memory } from "@/lib/wasm/metadocu_parser_wasm";
...
MetaDocu 深度剥离、自检与自动回滚的 Rust 底层核心生产级别实现（详见 [lib.rs](../src-wasm/src/lib.rs)）：
```rust
